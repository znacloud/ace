...
scene_url = "https://sagemaker-studio-163033260074-0o5wme941ix9.s3.ap-southeast-2.amazonaws.com/blacksofa.zip"
scene_file = "blacksofa.zip"
scene_name = "blacksofa"
...
def test_cv_split(idx, image_name, total):
    if image_name.startswith("cv_"):
        return ACE_DIRS.test
    else:
        return ACE_DIRS.train


def test_hv_split(idx, image_name, total):
    if image_name.startswith("hv_"):
        return ACE_DIRS.test
    else:
        return ACE_DIRS.train


def test_base_split(idx, image_name, total):
    if image_name.startswith("base_"):
        return ACE_DIRS.test
    else:
        return ACE_DIRS.train


def test_lv_split(idx, image_name, total):
    if image_name.startswith("lv_"):
        return ACE_DIRS.test
    else:
        return ACE_DIRS.train
...
SCENE_SPLIT_TYPES = ["icv", "ihv", "ibase", "ilv", "i50r"]
